    ...
    while True:
        ...
        for it , data in tqdm.tqdm(enumerate(dataloader), total=int(len(dataloader))):
            ...
            if time.time() - lastUpdateTime > 60 :
                lastUpdateTime = time.time()
                _,_,_ =  logStep(iter)
                extremeImages = images[ [trainInfo.bestRealIndex
                                        ,trainInfo.worstRealIndex
                                        ,trainInfo.bestFakeIndex
                                        ,trainInfo.worstFakeIndex
                                        ,trainInfo.lowestDifIndex
                                        ,trainInfo.highestDifIndex
                                        ], ... ]
                extremeImages = torch.cat( (extremeImages, refImages[[0],...]), dim=0 )
                collageE, extprobs, _ = generateDiffImages(extremeImages, layout=0)
                collageS = np.zeros( (2*DCfg.sinoSh[1] + DCfg.gapW ,
                                     4*DCfg.sinoSh[0] + 3*DCfg.gapW), dtype=np.float32  )
                def addImage(clmn, row, img, stretch=True) :
                    collageS[ row * ( DCfg.sinoSh[1]+DCfg.gapW) : (row+1) * DCfg.sinoSh[1] + row*DCfg.gapW ,
                              clmn * ( DCfg.sinoSh[0]+DCfg.gapW) : (clmn+1) * DCfg.sinoSh[0] + clmn*DCfg.gapW ] = \
                        img.squeeze().detach().cpu().numpy()
                addImage(0,0,collageE[0,0])
                addImage(0,1,collageE[1,0])
                addImage(1,0,collageE[2,2])
                addImage(1,1,collageE[3,2])
                addImage(2,0,collageE[-1,2])
                addImage(2,1,collageE[-1,0])
                addImage(3,0,collageE[-1,1])
                addImage(3,1,collageE[-1,3])
                ...
                print (f"TT: {extprobs[0,0]:.4e} ({data[1][trainInfo.bestRealIndex]},{data[2][trainInfo.bestRealIndex]}),  "
                       f"FT: {extprobs[2,2]:.4e} ({data[1][trainInfo.bestFakeIndex]},{data[2][trainInfo.bestFakeIndex]}),  "
                       f"GP: {extprobs[-1,2].item():.5f}, {extprobs[-1,1].item():.5f} " )
                print (f"TF: {extprobs[1,0]:.4e} ({data[1][trainInfo.worstRealIndex]},{data[2][trainInfo.worstRealIndex]}),  "
                       f"FF: {extprobs[3,2]:.4e} ({data[1][trainInfo.worstFakeIndex]},{data[2][trainInfo.worstFakeIndex]}),  "
                       f"R : {extprobs[-1,0].item():.5f} ({data[1][trainInfo.highestDifIndex]},{data[2][trainInfo.highestDifIndex]})" )
                plotImage(collageS)
