...
def loss_Gen(p_true, p_pred):
    ...
    sumweights = 0
    individualLosses = {}
    p_pred_gapped = p_true.clone().detach()
    p_pred_gapped[DCfg.gapRng] = p_pred[DCfg.gapRng]
    p_pred = p_pred_gapped
    ...
    def summarizeImages(images) :
        ...
        for i in range(batchSplit) :
            ...
            patchImages = ( generator.preFill(subImages) \
                            if onPrep else \
                            generator.forward(subImages) )[0][DCfg.gapRng]
        ...
        if 'Adv' in metrices and metrices['Adv'].weight > 0 : # discriminator
...
def generateDisplay(inp=None, boxes=None) :
    ...
    views = torch.empty((nofIm, 4, *DCfg.viewSh ), dtype=torch.float32, device=TCfg.device)
    with torch.no_grad() :
        prePatches = generator.preFill(images)[0][DCfg.gapRng]
        preImages[DCfg.gapRng] = prePatches
        genPatches = generator.forward(images)[0][DCfg.gapRng]
    ...
    if inp is None :
        ...
        boxes = torch.zeros((nofIm,), dtype=torch.int, device=images.device)
    elif boxes is None : # find worst boxes
        diffImages = torch.abs(genImages - images)
        diffY = fn.conv2d(diffImages, torch.ones((1,1,*DCfg.viewSh), device=diffImages.device))
        boxes = diffY.squeeze(1,-1).argmax(dim=-1)

    for curim in range(nofIm) :
        rng = np.s_[curim, 0, boxes[curim] : boxes[curim] + DCfg.viewSh[-2], : ]
        ...
        views[curim,3,:,DCfg.gapW:2*DCfg.gapW] = (images[DCfg.gapRng] - prePatches)[rng]
        views[curim,3,:,-2*DCfg.gapW:-DCfg.gapW] = (images[DCfg.gapRng] - genPatches)[rng]
    ...
    while TCfg.nofEpochs is None or epoch <= TCfg.nofEpochs :
        ...
        for it , data in tqdm.tqdm(enumerate(trainLoader), total=int(len(trainLoader))):
            ...
            if math.isnan(trainRes.lossG):
                raise Exception("Oops, Eagle did it again!")
            resAcc += trainRes
            updAcc += trainRes
