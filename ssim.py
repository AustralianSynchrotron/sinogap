# Comes from https://github.com/jinhwanlazy/pytorch-msssim.git
# Modified to solve some bugs and allow per-channel mask support
...
def _ssim(
    ...
    if mask is not None:
        nofIm = mask.shape[0]
        ssim_per_channel = [ torch.masked_select(ssim_map[im,...], mask[im,...]).mean() \
                             for im in range(nofIm) ]
        ssim_per_channel = torch.stack(ssim_per_channel)
        #ssim = torch.masked_select(ssim_map, mask).mean()
        cs = [ torch.masked_select(cs_map[im,...], mask[im,...]).mean() \
                             for im in range(nofIm) ]
        cs = torch.stack(cs)
