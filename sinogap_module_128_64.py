...
class DCfgClass:
    ...
    def __post_init__(self):
        self.readSh : tuple = (128 if self.brick else None ,64)
        self.sinoSh = ( (8 if self.brick else 256) * self.gapW , 4*self.gapW )
        self.viewSh = ( 8 * self.gapW , 4*self.gapW )
...
def generateDisplay(inp=None, boxes=None) :
    ...
    for curim in range(nofIm) :
        ...
        views[curim,3,:,:DCfg.gapW] = (images[DCfg.gapRng] - prePatches)[rng]
        views[curim,3,:,-DCfg.gapW:] = (images[DCfg.gapRng] - genPatches)[rng]
